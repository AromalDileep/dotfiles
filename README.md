...
niri is used as a **minimal, keyboard-driven Wayland compositor** on Fedora-based systems.

### Design Principles

* Declarative configuration only
* No compositor-managed bars by default
* No auto-spawned UI components
* External tools remain optional and explicitly enabled
...
* `config.kdl` is safe to track and stow across machines
* The file is inert unless niri is actually installed and running
* Waybar is **not auto-started** (`spawn "waybar"` intentionally disabled)
* Runtime, state, and socket directories are intentionally excluded

This ensures:

* Fedora systems using niri get a clean, predictable setup
* Ubuntu and non-niri systems are unaffected
* No visual artifacts or layout conflicts from foreign bars

niri configuration is only stowed on systems where niri is actively used.
